...
p1 <- ggsurv(sf.cancer, CI = T, surv.col = 'black', cens.col = 'red', size.est = 0.6, size.ci = 0.5) +
  geom_line(data = d1, aes(x, y, colour = "Censored"), size = 0.6) +
  scale_color_manual(name = "Type", values = c("Censored" = "blue")) +
  theme(legend.position = c(0.8, 0.8))


p2 <- ggsurv(sf.cancer, CI = T, surv.col = 'black', cens.col = 'red', size.est = 1, size.ci = 0) +
